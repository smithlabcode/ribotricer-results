...
psite_file = "/home/cmb-panasas2/wenzhenl/benchmark/{}/{}/data_tracks/P_sites_all_tracks_ccds".format(
    ref, pro)
center_file = "/home/cmb-panasas2/wenzhenl/benchmark/{}/{}/data_tracks/Centered_RNA_tracks_ccds".format(
    ref, pro)
frame_file = "/home/cmb-panasas2/wenzhenl/genomes/{}/annotation/ribotaper_annotation/frames_ccds".format(
    ref)
...
def read_track(filename):
    ...
    with open(filename) as fin:
        for line in fin:
            chrom, start, end, type, geneid, strand, psite = line.strip(
            ).split("\t")
...
def extract_frame(orf_psite_array):
    ...
    if orf_psite_array.size % 3 != 0:
        ...
        f0 = orf_psite_array[:-shiftn][np.arange(
            0, orf_psite_array.size - shiftn, 3)]
        f1 = orf_psite_array[:-shiftn][np.arange(
            1, orf_psite_array.size - shiftn, 3)]
        f2 = orf_psite_array[:-shiftn][np.arange(
            2, orf_psite_array.size - shiftn, 3)]
    else:
...
for name, psites in psite_dict.iteritems():
    ...
    try:
        ...
        predict_frame = np.nan
        ribo_pvalue = np.nan
        ribo_coverage = np.nan
    ...
    try:
        ...
        predict_frame_rna = np.nan
        rna_pvalue = np.nan
        rna_coverage = np.nan

    outlist = [
        name, annot_frame, predict_frame, ribo_pvalue, rna_pvalue,
        ribo_coverage, rna_coverage
    ]
